...
def map_thumb_key_to_qmk(tap, hold):
    if not hold:
...
def generate_qmk_layer(markdown_layer):
    rows = []
    for markdown_row in markdown_layer.main_rows:
        rows.append(",".join(map_key_label_to_qmk(label) for label in markdown_row))
    rows.append(
        ",".join(
            map_thumb_key_to_qmk(tap, hold) for tap, hold in markdown_layer.thumb_row
        )
    )
    return ",\n".join(rows)
...
def map_thumb_key_to_zmk(tap, hold):
    if not hold:
...
def generate_zmk_layer(markdown_layer):
    ...
    for markdown_row in markdown_layer.main_rows:
        ...
        rows.append(
            "&trans "
            + " ".join(map_key_label_to_zmk(label) for label in markdown_row)
            + " &trans"
        )
    rows.append(
        " ".join(
            map_thumb_key_to_zmk(tap, hold) for tap, hold in markdown_layer.thumb_row
        )
    )
    return "\n".join(rows)
